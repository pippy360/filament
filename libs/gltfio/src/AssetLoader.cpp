...
public:
    AssetConfigurationExtended* mConfigExtended = nullptr;
...
void FAssetLoader::createPrimitives(const cgltf_node* node, const char* name,
    ...
    for (cgltf_size index = 0, n = mesh->primitives_count; index < n; ++index) {
        ...
        if (!outputPrim.vertices) {
            if (mLoaderExtended) {
                fAsset->mResourceInfo = FFilamentAsset::ResourceInfoExtended{};
                ...
                utils::slog.e << "regular path" << utils::io::endl;
                fAsset->mResourceInfo = FFilamentAsset::ResourceInfo{};
