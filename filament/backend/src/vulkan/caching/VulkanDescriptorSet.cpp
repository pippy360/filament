...
public:
    ...
    inline bool canAllocate(VulkanDescriptorSetLayout* layout) {
        return layout->count == mCount && (mSize < mCapacity || mUnusedCount > 0);
    }
...
private:
    struct BiMap {
        using ForwardMap = std::unordered_map<Key, SetPtr, typename Key::HashFn,
                typename Key::Equal>;

        typename ForwardMap::const_iterator find(Key const& key) const { return forward.find(key); }
        typename ForwardMap::const_iterator end() const { return forward.end(); }

        inline size_t size() const { return forward.size(); }
        ...
        SetPtr const& get(Key const& key) { return forward[key]; }
...
// be re-used if necessary. Therefore, we'll cache a set based on its content and return a cached
