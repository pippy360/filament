...
VulkanProgram::VulkanProgram(VkDevice device, Program const& builder) noexcept
    ...
    for (size_t i = 0; i < MAX_SHADER_MODULES; i++) {
        ...
        utils::slog.e <<"stage=" << i << " u=" << printx(ubo)
                      <<" s=" << printx(sampler) <<" i=" << printx(inputAttachment) << utils::io::endl;
    ...
    utils::slog.e << "ubo=" << printx(uboMask) << " capacity=" << layouts[0].bindings.capacity()
                  << utils::io::endl;
    utils::slog.e << "sampler=" << printx(samplerMask)
                  << " capacity=" << layouts[1].bindings.capacity() << utils::io::endl;
    utils::slog.e << "inputAttachment=" << printx(inputAttachmentMask)
                  << " capacity=" << layouts[2].bindings.capacity() << utils::io::endl;
