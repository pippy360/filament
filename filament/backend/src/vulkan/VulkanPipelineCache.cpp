...
constexpr uint8_t SAMPLER_BINDING_COUNT = VulkanDescriptorSetManager::SAMPLER_BINDING_COUNT;

UsageFlags VulkanPipelineCache::getUsageFlags(uint16_t binding, ShaderStageFlags flags,
        UsageFlags src) {
    ...
    if (any(flags & ShaderStageFlags::FRAGMENT)) {
        src.set(SAMPLER_BINDING_COUNT + binding);
    }
...
//    utils::slog.e <<"layout=" << layout << utils::io::endl;
