...
struct VulkanDescriptorSetLayout : public VulkanResource {
    ...
    struct Bitmask {
        ...
        // Because we're using this struct as hash key, must make it's 8-bytes aligned, with not
        // unaccounted for bytes.
    ...
    // This is a convenience struct so that we don't have to iterate through all of the binding
    // indices (including unbound indices).
    struct _Bindings {
